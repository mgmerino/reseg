...
module Reseg
  module Core
    ...
    class Segment
      ...
      # @raise [ValidationError] if the starts_at value is invalid.
      # @raise [ValidationError] if the ends_at value is invalid.
      # @raise [ValidationError] if the ends_at value is less than the starts_at value.
      ...
      def validate_starts_at!(value)
        raise ValidationError, "starts_at must be a Time object" unless value.is_a?(Time)

        value
      end

      def validate_ends_at!(value)
        raise ValidationError, "ends_at must be a Time object" unless value.is_a?(Time)
        raise ValidationError, "ends_at must be >= starts_at" if starts_at && value < starts_at
